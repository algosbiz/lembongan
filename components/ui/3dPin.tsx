...
export const PinContainer = ({ children, titles, className, containerClassName }: { children: React.ReactNode; titles?: TitleLink[]; className?: string; containerClassName?: string }) => {
  ...
  const handleMouseEnter = () => {
    if (!isMobile) setTransform("translate(-50%,-50%) rotateX(40deg) scale(0.8)");
  };

  const handleMouseLeave = () => {
    if (!isMobile) setTransform("translate(-50%,-50%) rotateX(0deg) scale(1)");
  };

  const getTransform = () => {
    if (isMobile) {
      return isActive ? "translate(-50%,-50%) rotateX(40deg) scale(0.8)" : "translate(-50%,-50%) rotateX(0deg) scale(1)";
    }
    return transform;
  };

  return (
    <div className={cn("relative group/pin z-50 cursor-pointer", containerClassName)} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onClick={handleClick}>
      ...
      <PinPerspective titles={titles} isMobile={isMobile} isActive={isMobile ? isActive : false} />
    </div>
  );
};

export const PinPerspective = ({ titles, isMobile, isActive }: { titles?: TitleLink[]; isMobile: boolean; isActive: boolean }) => {
  const visibleClass = isMobile ? (isActive ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none") : "opacity-0 pointer-events-none group-hover/pin:opacity-100 group-hover/pin:pointer-events-auto";

  const shouldShowEffects = isMobile ? isActive : false; // ⬅ ripple only on hover in desktop (handled by CSS)

  return (
    <motion.div className={cn("w-96 h-96 flex items-center justify-center z-[60] transition-opacity duration-500", visibleClass)}>
      <div className="w-full h-full flex-none inset-0 relative">
        ...
        {/* Ripple and lines: only rendered when needed */}
        {(shouldShowEffects || !isMobile) && (
          <>
            {/* Ripple */}
